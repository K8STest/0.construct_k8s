...
kubectl get po -n redis # Pod 就都消失了

kubectl get pvc -n redis # 数据还在

kubectl delete pvc redis-data-redis-master-0 redis-data-redis-replicas-0 redis-data-redis-replicas-1 redis-data-redis-replicas-2 -n redis # 手动删除

pv 集群的资源不用指定命名空间，pvc 删掉后，PV 也就删掉了
