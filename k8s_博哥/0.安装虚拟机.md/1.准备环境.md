...
# 操作
dns 这些信息是从以前的文档中找出来的，为何如此配置呢？

```

bash init.sh
Input IP address(ip_address): 192.168.1.26 # 输入

Input IP address(ip_gateway): 192.168.1.1 # 输入

Input IP address(dns1_ip): 192.168.1.1 # 输入

Input IP address(dns2_ip): 182.254.116.116 # 输入
    ...
    ens32:
      ...
      routes:
        - to: default
          via: 192.168.1.1
      nameservers:
          addresses: [192.168.1.1, 182.254.116.116]
