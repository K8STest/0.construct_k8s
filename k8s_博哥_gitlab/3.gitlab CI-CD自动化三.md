...
https://www.toutiao.com/i6948405450794713612/
...
#              memory: 8Gi
          ...
          env:
            - name: TZ
              value: Asia/Shanghai
            - name: GITLAB_OMNIBUS_CONFIG  # 固定的配置
              value: |
                postgresql['enable'] = false  # 关闭内置的pg
                gitlab_rails['db_username'] = "gitlab"
                gitlab_rails['db_password'] = "bogeusepg"
                gitlab_rails['db_host'] = "postgresql"   # 引用之前部署的 pg的 svc服务
                ...
                external_url 'http://git.boge.com/' # 内部使用 http 80端口就可以
...
kubectl -n gitlab-ver130806 delete networkpolicies.networking.k8s.io --all

# 这里遇到了一个问题

配置完 ingress service 之后，加了 host 之后 ，浏览器 通过 git.boge.com 无法访问到 gitlab 页面
报错 503 Servicec temporarily unavailable

原因是 Ingress 端口配错成 8 了
