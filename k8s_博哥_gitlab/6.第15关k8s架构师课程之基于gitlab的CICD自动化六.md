...
https://www.toutiao.com/i6949848083358548494/
  ...
  template:
    metadata:
      labels:
        app: dind
    spec:
      hostNetwork: true # 容器端口映射到宿主机上，跟宿主机同一个网络了
      containers:
      - name: dind
        image: docker:19-dind
        #image: harbor.boge.com/library/docker:19-dind
        # image: registry.cn-hangzhou.aliyuncs.com/abelcontainer/docker:24-dind
        # 操作时候用的这个镜像
        lifecycle:
...
#            memory: 1Gi
        ...
        volumeMounts:
          ...
          - name: tz-config
           ...
           # 这里是这么创建的 /opt/k8s/harbor/tls/boge.com.cert
           #  Harbor 的 CA 证书挂载到容器内部以支持 Docker 登录 Harbor
          - name: harbor-ca
